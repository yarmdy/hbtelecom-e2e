...
namespace DWWinService
{
    class WebAnalyzer:Analyzer
    {
        ...
        public override void CombineRowQuarter(Dictionary<string, Dictionary<string, object>> dics, Dictionary<string, object> dic)
        {
            ...
            if (O2.O2I(dic_exist["WEB_RATE"]) < O2.O2I(dic["WEB_RATE"]))
            {
                dic_exist = dic;
            }
        }
        public override void CombineRowDay(Dictionary<string, Dictionary<string, object>> dics, Dictionary<string, object> dic)
        {
            ...
            dic_exist["WEB_RATE"] = O2.O2I(dic_exist["WEB_RATE"]) + O2.O2I(dic["WEB_RATE"]);
            dic_exist["WEB_COUNT"] = O2.O2I(dic_exist["WEB_COUNT"]) + O2.O2I(dic["WEB_COUNT"]);
            dic_exist["PAGE_RATE"] = O2.O2I(dic_exist["PAGE_RATE"]) + O2.O2I(dic["PAGE_RATE"]);
            dic_exist["SCREEN_RATE"] = O2.O2I(dic_exist["SCREEN_RATE"]) + O2.O2I(dic["SCREEN_RATE"]);
        }
        public override bool IsZhiCha(Dictionary<string, object> dic)
        {
            double webRate = 1;
            double first_open = O2.O2I(dic["SCREEN_RATE"]);
            webRate = 0.8 * (1-O2.O2I(dic["PAGE_RATE"]).Div0(O2.O2I(dic["WEB_COUNT"]))) + 0.2 * (1-O2.O2I(dic["WEB_RATE"]).Div0(first_open));
            //webRate = 0.8 * (1 - O2.O2I(dic["PAGE_RATE"]).Div(O2.O2I(dic["WEB_COUNT"]))) + 0.2 * O2.O2I(dic["WEB_RATE"]).Div(O2.O2I(dic["SCREEN_RATE"]));
            if (first_open >= 500 && webRate < 0.8)
                return true;
            return false;
        }
        ...
        {
            double webRate = 1;
            webRate = 0.8 * (1 - O2.O2I(dic["PAGE_RATE"]).Div0(O2.O2I(dic["WEB_COUNT"]))) + 0.2 * (1-O2.O2I(dic["WEB_RATE"]).Div0(O2.O2I(dic["SCREEN_RATE"])));
            //webRate = 0.8 * (1 - O2.O2I(dic["PAGE_RATE"]).Div(O2.O2I(dic["WEB_COUNT"]))) + 0.2 * O2.O2I(dic["WEB_RATE"]).Div(O2.O2I(dic["SCREEN_RATE"]));
            if (webRate < 0.8 && O2.O2I(dic["SCREEN_RATE"]) >= 10000)
                return true;
            return false;
        }
        ...
        public override void AfterImportDB(String time)
        {
            ...
            string tm = DateTime.Parse(time).AddMinutes(-15).ToString("yyyy-MM-dd HH:mm");
