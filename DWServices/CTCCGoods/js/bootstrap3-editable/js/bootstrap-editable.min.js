...
(function ($) {
    ...
    var SelectX = function (options) {
        this.init('selectx', options, SelectX.defaults);
    };

    $.fn.editableutils.inherit(SelectX, $.fn.editabletypes.list);
    ...
    $.extend(SelectX.prototype, {
        ...
        renderList: function () {
            ...
            var fillItems = function ($el, data) {
                //if ($el.hasClass("slt1")) return $el;
                var attr;
                if ($.isArray(data)) {
                    //var $option2 = $('<option>', { value: -1 });
                    //$option2[escape ? 'text' : 'html']("all");
                    //$el.eq(0).append($option2);
                    for (var i = 0; i < data.length; i++) {
                        attr = {};
                        if (data[i].children) {
                            attr.label = data[i].text;
                            $el.eq(1).append(fillItems($('<optgroup>', attr), data[i].children));
                        } else {
                            attr.value = data[i].value;
                            attr.fenlei = data[i].fenlei;
                            if (data[i].disabled) {
                                attr.disabled = true;
                            }
                            ...
                            if (data[i].fenlei) {
                                var has = false;
                                $el.eq(0).find("option").each(function () {
                                    has = has || $(this).text() == data[i].fenlei;
                                });
                                if (!has) {
                                    var op1 = $("<option>" + data[i].fenlei + "</option>");
                                    if (data[i].value == -1) {
                                        op1.val(-1);
                                    }
                                    $el.eq(0).append(op1);
                ...
                $($el.eq(0)).on("change", function () {
                    var text = $(this).find("option:selected").text();
                    var v = $(this).find("option:selected").val();
                    $el.eq(1).find("option").show();
                    if (v == -1) return;
                    $el.eq(1).find("option").each(function () {
                        if ($(this).attr("fenlei") != text && $(this).val() != -1) {
                            $(this).hide();
                        }
                    });
                });
    ...
    SelectX.defaults = $.extend({}, $.fn.editabletypes.list.defaults, {
        /**
        @property tpl 
        @default <select></select>
        **/
        tpl: '<select></select><select></select>'
    });

    $.fn.editabletypes.selectx = SelectX;
